...
<?php 
		...
		<?php
			...
			$og = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(*) AS tea FROM post LEFT JOIN user ON post.postAuthor = user.uid"))['tea'];
			...
			$go = mysqli_query($conn, $query);
				...
				?>
						<div class="article-card">
							...
							<p class="article-author"><a href="profil.php?uid=<?= $el['uid'];?>"><?= $el['username'];?></a></p>
						</div>
	...
	<?php
			...
			<?php }}else{?>
					...
					<p>Pencarian tidak ditemukan</br>Maaf, tidak ada hasil pencarian unutk kata kunci <b><?= $katakunci; ?></b>. </br> </br> 1. Cobalah menggunakan kata kunci umum. Misalkan: Jika kamu ingin mencari 'Cara menanam padi' maka gunakan kata kunci umum seperti 'menanam', 'padi' </br></br> 2. Coba gunakan kata lain yang mempunyai makna yang sama </br></br> 3. Periksa kembali penulisan kata kunci</p>
	...
	<div id="pagination">
		<?php
			if(isset($_GET['c'])){
				...
				for($z=1; $z<=$pages; $z++){
					echo ($z == $page) ? '<div class="selectedpage"><a>'.$z.'</a></div>' : '<div class="numberpage"><a href="index.php?cari='.$katakunci.'&hlm='.$z.'&c=Cari">'.$z.'</a></div>';
				}
			}
		}else{
			if($pages >= 1 && $page <= $pages){
				for($z=1; $z<=$pages; $z++){
					echo ($z == $page) ? '<div class="selectedpage"><a>'.$z.'</a></div>' : '<div class="numberpage"><a href="index.php?hlm='.$z.'">'.$z.'</a></div>';
				}
			}
		}
?>
